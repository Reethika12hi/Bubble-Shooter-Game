...
window.onload = function () {
    ...
    // Score
    var score = 0;
    var highscore = 0; // **FEATURE: HIGH SCORE**
    ...
    // **FEATURE: PREDICTION LINE**
    var shootline = {
        x: 0,
        y: 0,
        angle: 0,
        visible: false,
        coords: [] // Store line segments for rendering
    };
    ...
    function init() {
        // Load images
        images = loadImages(["assets/images/bubble-sprites.png"]);
        ...
        // **FEATURE: HIGH SCORE** - Load high score from local storage
        loadHighScore();
    ...
    // **FEATURE: HIGH SCORE** - Function to load high score
    function loadHighScore() {
        if (typeof (Storage) !== "undefined") {
            var storedScore = localStorage.getItem("bubbleShooterHighScore");
            if (storedScore) {
    ...
    // **FEATURE: HIGH SCORE** - Function to save high score
        ...
        if (animationstate == 1) {
            ...
            if (!tilesleft) {
                ...
                if (tilefound) {
                    ...
                    // **FEATURE: HIGH SCORE** - Save high score
                    saveHighScore();
    ...
    function render() {
        ...
        // **FEATURE: HIGH SCORE** - Draw high score
        context.fillStyle = "#0fd637ff";
        context.font = "bold 18px Verdana";
        var highscorex = level.x + 0;
        var highscorey = level.y + level.height + level.tileheight - yoffset - 8;
        drawCenterText("High Score:", highscorex, highscorey, 150);
        context.font = "24px Verdana";
        drawCenterText(highscore, highscorex, highscorey + 30, 150);
        ...
        // **FEATURE: PREDICTION LINE** - Render the prediction line
        if (gamestate == gamestates.ready) {
            renderShootLine();
        }
        ...
        if (gamestate == gamestates.gameover) {
            ...
            drawCenterText("Your Score: " + score, level.x, level.y + level.height / 2 + 40, level.width);
            drawCenterText("High Score: " + highscore, level.x, level.y + level.height / 2 + 70, level.width);
            drawCenterText("Click to start", level.x, level.y + level.height / 2 + 100, level.width);
        }
    }
    ...
    function drawFrame() {
        ...
        // Draw title
        context.fillStyle = "#da1a93ff"; // Set text color
        context.font = " bold 20px sans-serif";
        context.fillText("BUBBLE SHOOTER", 195, 30);


        // Display fps
        context.fillStyle = "#a0efccff";
    ...
    // **FEATURE: PREDICTION LINE** - Draw the prediction line
    function renderShootLine() {
        var startx = player.x + level.tilewidth / 2;
        var starty = player.y + level.tileheight / 2;
        ...
        var lineCoords = calculateShootLine(startx, starty, angle, 3); // Max 3 bounces

        context.beginPath();
        context.moveTo(startx, starty);

        for (var i = 0; i < lineCoords.length; i++) {
            context.lineTo(lineCoords[i].x, lineCoords[i].y);
        }

        context.stroke();
        context.setLineDash([]); // Reset line to solid
    }

    // **FEATURE: PREDICTION LINE** - Function to calculate the path of the bubble
    function calculateShootLine(x, y, angleRad, bounces) {
        var path = [];
        var currentX = x;
        var currentY = y;
        var currentAngle = angleRad;
        var radius = level.radius;
        var maxDistance = 10000; // Large distance to trace

        for (var i = 0; i < bounces; i++) {
            var dx = Math.cos(currentAngle);
            var dy = -Math.sin(currentAngle); // Y is inverted

            // 1. Calculate intersection with walls
            var wallHit = {
                x: currentX + dx * maxDistance,
                y: currentY + dy * maxDistance,
                isWall: false
            };

            // Check for left wall
            if (dx < 0 && currentX + dx * maxDistance < level.x + radius) {
                var distance = (level.x + radius - currentX) / dx;
                wallHit.x = level.x + radius;
                wallHit.y = currentY + dy * distance;
                wallHit.isWall = true;
                currentAngle = Math.PI - currentAngle; // Reflect angle
            }
            // Check for right wall
            else if (dx > 0 && currentX + dx * maxDistance > level.x + level.width - radius) {
                var distance = (level.x + level.width - radius - currentX) / dx;
                wallHit.x = level.x + level.width - radius;
                wallHit.y = currentY + dy * distance;
                wallHit.isWall = true;
                currentAngle = Math.PI - currentAngle; // Reflect angle
            }

            // 2. Calculate intersection with other tiles or top
            var closestTileHit = null;
            var closestDistance = Infinity;

            // Check top collision
            var topDistance = (level.y + radius - currentY) / dy;
            if (dy < 0 && topDistance > 0 && topDistance < closestDistance) {
                closestTileHit = {
                    x: currentX + dx * topDistance,
                    y: level.y + radius,
                    isTile: false
                };
            ...
            // Check collisions with existing tiles
            for (var col = 0; col < level.columns; col++) {
                for (var row = 0; row < level.rows; row++) {
                    var tile = level.tiles[col][row];
                    if (tile.type < 0) continue;

                    var coord = getTileCoordinate(col, row);
                    var bubbleCenterX = coord.tilex + level.tilewidth / 2;
                    var bubbleCenterY = coord.tiley + level.tileheight / 2;

                    // Simple check: Check if the line segment intersects the circle
                    // This is an approximation but good enough for a prediction line
                    var hitPoint = lineCircleIntersection(currentX, currentY, wallHit.x, wallHit.y, bubbleCenterX, bubbleCenterY, 2 * radius);

                    if (hitPoint.hit) {
                        var distToTile = Math.sqrt(Math.pow(hitPoint.x - currentX, 2) + Math.pow(hitPoint.y - currentY, 2));
            ...
            // Determine the actual segment end point
            var segmentEnd = wallHit;
            var segmentLength = Math.sqrt(Math.pow(wallHit.x - currentX, 2) + Math.pow(wallHit.y - currentY, 2));

            if (closestTileHit && closestDistance < segmentLength) {
                // Collision with tile or top is closer
                segmentEnd.x = currentX + dx * closestDistance;
                segmentEnd.y = currentY + dy * closestDistance;

                // Move the point back a bit to be inside the collision circle for visualization
                segmentEnd.x -= dx * 1;
                segmentEnd.y -= dy * 1;

                path.push({ x: segmentEnd.x, y: segmentEnd.y });
                return path; // The bubble will stop here, so we end the line
            }

            // Add the segment end to the path
            path.push({ x: segmentEnd.x, y: segmentEnd.y });

            // Set the new start point for the next bounce
            currentX = segmentEnd.x;
            currentY = segmentEnd.y;

            // If it hit a wall, the loop will continue with the new angle.
            // If it didn't hit a wall (it went to maxDistance), we stop.
            if (!wallHit.isWall) {
                break;
            }
        }
    ...
    // Helper function for prediction line: Line segment / Circle intersection
    function lineCircleIntersection(x1, y1, x2, y2, cx, cy, r) {
        ...
        // Check if collision point is on the line segment (0 <= t <= 1)
        if ((t1 >= 0 && t1 <= 1) || (t2 >= 0 && t2 <= 1)) {
            var t = (t1 >= 0 && t1 <= 1) ? t1 : t2; // Prefer the closer (smaller t) point that is on the segment
